...
for directory in categoryDirectory.GetListOfKeys():
    ...
    theBackgroundStack.GetYaxis().SetTitleOffset(0.3)
    theBackgroundStack.GetYaxis().SetTitleSize(0.08)
    theBackgroundStack.GetYaxis().CenterTitle()
    ...
    verticalLocation = max(theBackgroundStack.GetMaximum(),histograms['data'].GetMaximum())*2
    if args.measurementType == 'pth':
        if channel == 'tt' and category == 'HighTauPt':
            latex.DrawLatex(6,verticalLocation,'0<p_{t}^{H}<80')
            latex.DrawLatex(6+12,verticalLocation,'80<p_{t}^{H}<120')
            latex.DrawLatex(6+24,verticalLocation,'120<p_{t}^{H}<200')
            latex.DrawLatex(6+36,verticalLocation,'200<p_{t}^{H}<350')
            latex.DrawLatex(6+48,verticalLocation,'350<p_{t}^{H}<450')
            latex.DrawLatex(6+60,verticalLocation,'450<p_{t}^{H}')
        elif (channel == 'mt' or channel == 'et') and (category == 'LowTauPt' or category=='IntermediateTauPt'):
            latex.DrawLatex(6,verticalLocation,'0<p_{t}^{H}<45')
            latex.DrawLatex(6+12,verticalLocation,'45<p_{t}^{H}<80')
            latex.DrawLatex(6+24,verticalLocation,'80<p_{t}^{H}<120')
            latex.DrawLatex(6+36,verticalLocation,'120<p_{t}^{H}<200')
            latex.DrawLatex(6+48,verticalLocation,'200<p_{t}^{H}<350')
            latex.DrawLatex(6+60,verticalLocation,'350<p_{t}^{H}')
        else:
            latex.DrawLatex(6,verticalLocation,'0<p_{t}^{H}<45')
            latex.DrawLatex(6+12,verticalLocation,'45<p_{t}^{H}<80')
            latex.DrawLatex(6+24,verticalLocation,'80<p_{t}^{H}<120')
    ...
    if args.measurementType == 'ljpt':
        if channel == 'tt':
            ...
            latex.DrawLatex(6+12,verticalLocation,'60<p_{t}^{Jet}<120')
            ...
            latex.DrawLatex(6+24,verticalLocation,'60<p_{t}^{Jet}<120')
            latex.DrawLatex(6+36,verticalLocation,'120<p_{t}^{Jet}<200')
            latex.DrawLatex(6+48,verticalLocation,'200<p_{t}^{Jet}<350')
            latex.DrawLatex(6+60,verticalLocation,'350<p_{t}^{Jet}')

    #okay, let's force a grid on top
    plotGridPad = ROOT.TPad('slices','slices',0,0,1,1)    
    plotGridPad.SetLeftMargin(0.05)
    plotGridPad.SetRightMargin(0.05)
    plotGridPad.Draw()
    plotGridPad.cd()
    ...
    theLegend = ROOT.TLegend(0.07,0.75,0.93,0.88)
    ...
    theLegend.AddEntry(theStackErrors,'Unc.','F')
    theLegend.AddEntry(histograms['signal'],'H #rightarrow #tau#tau','F')    

    theLegend.SetNColumns(8)
    ...
    if args.lowerPad == 'ratio':
        ...
        ratioPlotErrors.GetXaxis().SetLabelSize(0)
        #setMassBinAxisTitles(ratioPlotErrors,rebinScheme)
        ratioPlotErrors.GetXaxis().SetTitle('m_{#tau#tau}')
        ratioPlotErrors.GetXaxis().SetTitleSize(0.2)
        ratioPlotErrors.GetXaxis().SetTitleOffset(0.3)
        ...
        unitRatioError.GetXaxis().SetLabelSize(0)
        #setMassBinAxisTitles(unitRatioError,rebinScheme)
        unitRatioError.GetXaxis().SetTitle('m_{#tau#tau}')
        unitRatioError.GetXaxis().SetTitleSize(0.2)
        unitRatioError.GetXaxis().SetTitleOffset(0.3)
    ...
    ratioGridPad.SetLeftMargin(0.05)
    ratioGridPad.SetRightMargin(0.05)
    ratioGridPad.Draw()
    ratioGridPad.cd()
    ...
    #floating axis
    axis1 = ROOT.TGaxis(0.0,-0.2,12,-0.2,50,290,6,"+LN");
    axis1.SetTickLength(0.3)
    axis1.SetTickSize(0.3)
    axis1.SetLabelSize(0.11)
    axis1.Draw();
